...
export async function GET() {
    try {
        ...
        for (const d of (saleDetails ?? []) as any[]) {
            salesVelocity[d.item_id] = (salesVelocity[d.item_id] ?? 0) + d.quantity
        }

        const recommendations = stockData.map((s: any) => {
            ...
            const recommendedQty = Math.max(
                Math.ceil(avgDaily * 14) - s.current_stock, // 2 weeks worth
                5,
            )

            const urgency = s.current_stock <= 2 ? 'critical' : s.current_stock <= 5 ? 'high' : 'medium'
