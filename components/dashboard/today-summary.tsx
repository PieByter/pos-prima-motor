      ...
      {isLoading ? (
        ...
        <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1">
          <span className="flex items-center gap-1 text-sm font-semibold text-slate-900 dark:text-white">
            ...
            {t("dashboard.transactionCount", { n: data?.transactionCount ?? 0 })}
          </span>
          <span className="flex items-center gap-1 text-sm font-semibold text-slate-900 dark:text-white">
          ...
          {data && data.lowStockCount > 0 && (
            <span className="flex items-center gap-1 text-xs font-medium text-amber-600">
              ...
              {t("dashboard.criticalItems", { n: data.lowStockCount })}
            </span>
          )}
          {data && (data.unreadNotifications ?? 0) > 0 && (
            <span className="flex items-center gap-1 text-xs font-medium text-red-600">
              <Bell className="h-3 w-3" />
              {t("dashboard.newNotifications", { n: data.unreadNotifications ?? 0 })}
            </span>
          )}
        </div>
      )}
