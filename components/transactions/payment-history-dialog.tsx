  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        if (!res.ok) throw new Error(t("paymentHistory.loadFailed"));
        const data = (await res.json()) as SalePaymentWithMethod[];
        setPayments(Array.isArray(data) ? data : []);
      } catch (err) {
        if (!cancelled) setError(err instanceof Error ? err.message : t("paymentHistory.loadFailedShort"));
      } finally {
  ...
  async function handleDelete(paymentId: number) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.error ?? t("paymentHistory.deleteFailed"));
      }
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-lg">{t("paymentHistory.title")}</DialogTitle>
              <DialogDescription className="mt-0.5 text-sm">
                {payments.length > 0
                  ? t("paymentHistory.totalPaid", { amount: formatRupiah(totalPaid), count: payments.length })
        ...
        <div className="space-y-2 max-h-80 overflow-y-auto pt-2">
          {isLoading ? (
            ...
            <p className="text-center text-sm text-slate-400 py-6">{t("paymentHistory.noPaymentsShort")}</p>
          ) : (
            payments.map((p) => (
              <div
                key={p.id}
